...
| :--- | :--- | :--- | :--- |
| Java (Gradle) | <nobr>デフォルトで**ブロック**</nobr> | <nobr>─（HTTP 接続自体が不可）</nobr> | `allowInsecureProtocol = true` で HTTP を許可 |
| Java (Maven) | <nobr>**許可**（デフォルト）</nobr> | **許可**（デフォルト） | 特別な設定不要 |
| JavaScript (npm) | **許可**（デフォルト） | デフォルトで**ブロック** | `always-auth = true` で HTTP にも認証情報を送信 |
